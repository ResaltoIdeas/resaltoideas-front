...
export function CoverflowCarousel({
  ...
  const clamp = React.useCallback(
    (pos: number) => (loop ? pos : Math.max(0, Math.min(Math.max(count - 1, 0), pos))),
    [count, loop],
  );
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!drag.moved && count > 0) {
      const hit = (event.target as HTMLElement | null)?.closest("[data-slide-index]");
      if (hit) {
        const index = Number(hit.getAttribute("data-slide-index"));
        if (Number.isFinite(index)) {
          if (index === indexAt(posRef.current)) {
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-rose focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-[4/5] overflow-hidden rounded-2xl bg-rose-wash shadow-rose will-change-transform",
                  cardClassName,
                )}
              ...
              >
                {slide.src ? (
                  ...
                  <div
                    className={cn(
                      "flex h-full w-full items-end bg-gradient-to-br p-5",
                      slide.tone ?? "from-[#f6bfd2] to-[#e78cab]",
                    )}
                  >
                    <span className="rounded-lg bg-white/85 px-3 py-1.5 text-sm font-medium text-ink">
                      {slide.title ?? "Preview"}
                    </span>
                  </div>
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-lg border border-[var(--line)] bg-surface/90 p-2 text-ink shadow-rose backdrop-blur transition hover:bg-rose-wash"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-lg border border-[var(--line)] bg-surface/90 p-2 text-ink shadow-rose backdrop-blur transition hover:bg-rose-wash"
            >
      ...
      {showCaption && active?.title && (
        <div key={selected} className="cf-fade mt-2 flex flex-col items-center px-6">
          <p className="font-display text-[15px] font-semibold tracking-tight text-ink">
            {active.title}
          </p>
          {active.subtitle && (
            <p className="mt-1 text-[13px] font-semibold tabular-nums text-muted">
              {active.subtitle}
            </p>
          )}
